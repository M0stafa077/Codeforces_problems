...
Petya is sure about the problem's solution, otherwise he isn't sure. 
The second number shows Vasya's view on the solution, the third number shows 
...
int main(){
    int n;
    scanf("%d", &n);
    int _no_of_problems[n][3];
    int count = 0;
 
    if (1 <= n && 1000 >= n){
        for (int i = 0; i < n; i++)
        {
            scanf("%d %d %d", &_no_of_problems[i][0], &_no_of_problems[i][1], &_no_of_problems[i][2]);      
        }
        for (int i = 0; i < n; i++)
        {
            if( (_no_of_problems[i][0] + _no_of_problems[i][1] + _no_of_problems[i][2]) >= 2){
                ++count;
            }         
        }
        printf("%d", count);
